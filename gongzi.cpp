...
void read()/*从文件中读车数据。若无文件，请建立一个*/
{
	...
	int i=0,m;
       ...
       {
		    printf("文件打开失败\n");/*若单独的exe文件无法打开，请把gx.dat文件和exe放在一个文件夹*/
            getch();
            exit(-1);
       ...
       {
			 fscanf(fp,"%s %s %f %f %f %f",zggz[i].num,zggz[i].name,&(zggz[i].gwgz),&(zggz[i].xjsz),&(zggz[i].zwjt),&(zggz[i].xjgz));
